# Program to convert THB to USD and vice versa
...
class Currency_Converter:
    def __init__(self, mainWindow):
        ...
        self.input = tk.Entry(mainWindow, textvariable = self.money, width = 48, justify = "right")
        self.input.grid(row = 1, column = 1)

        self.usd_to_thb = tk.Button(mainWindow, text = "USD -> THB", width = 40, command = self.toTHB)
        self.usd_to_thb.grid(row = 2, column = 1)

        self.thb_to_usd = tk.Button(mainWindow, text = "THB -> USD", width = 40, command = self.toUSD)
